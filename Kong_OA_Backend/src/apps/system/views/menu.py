...
async def add_menu(
    ...
    await Menu.create(**menu.dict(), create_by=user.username, update_by=user.username)
    return APIResponse()
...
async def delete_menu(
    ...
    res = await Menu.filter(id__in=ids).update(is_delete=True, update_by=user.username)  # 软删除，字段控制
...
async def update_menu(
    ...
    await Menu.filter(id=menu_id).update(**menu.dict(), update_by=user.username)
    return APIResponse()
